...
root_img = '/dataset/AOLP/Subset_LE/Subset_LE/Subset_LE/Image'
root_anno = '/dataset/AOLP/Subset_LE/Subset_LE/Subset_LE/groundtruth_localization'
target_anno = '/data/AOLP/AOLP_LE/Annotations'

two_carplates = '/dataset/AOLP/Subset_LE/Subset_LE/Subset_LE/two_plates.txt'

two_carplates_arr = []

fi = open(two_carplates)
lines = fi.readlines()
fi.close()

# 所有两个车牌的图片
for line in lines:
    two_carplates_arr.append(int(line.strip()))
...
for fileitem in file_list:
    if fileitem.endswith('txt') and '_' not in fileitem:  # 确保不是第二个车牌
        ...
        # 因为298到351顺序正好循环错了一位
        if img_index == 298:
            dic['img_index'] = str(351)
        elif img_index >= 299 and img_index <= 351:
            dic['img_index'] = str(img_index - 1)
        else:
        ...
        if img_index in two_carplates_arr:
            fi = open(os.path.join(root_anno, str(img_index) + '_2.txt'))
            lines = fi.readlines()
            fi.close()
            xmin.append(int(float(lines[0].strip())))
            ymin.append(int(float(lines[1].strip())))
